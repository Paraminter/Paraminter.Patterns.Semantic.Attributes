namespace Attribinter.Patterns.Semantic.NonNullableArgumentPatternCases.FloatCases;

using Microsoft.CodeAnalysis;

using Xunit;

public sealed class TryMatch
{
    private static ArgumentPatternMatchResult<float> Target(IArgumentPattern<TypedConstant, float> pattern, TypedConstant argument) => pattern.TryMatch(argument);

    private static readonly PatternContext Context = PatternContext.Create();

    [Fact]
    public void FloatAttribute_Successful()
    {
        var source = """
            [Attribinter.Float(3.14f)]
            public class Foo { }
            """;

        Successful(3.14f, source);
    }

    [Fact]
    public void ObjectAttribute_Float_Successful()
    {
        var source = """
            [Attribinter.NonNullableObject(3.14f)]
            public class Foo { }
            """;

        Successful(3.14f, source);
    }

    [Fact]
    public void ObjectAttribute_Int_Unsuccessful()
    {
        var source = """
            [Attribinter.NonNullableObject(1)]
            public class Foo { }
            """;

        Unsuccessful(source);
    }

    [Fact]
    public void ObjectAttribute_String_Unsuccessful()
    {
        var source = """
            [Attribinter.NonNullableObject("")]
            public class Foo { }
            """;

        Unsuccessful(source);
    }

    [AssertionMethod]
    private static void Successful(float expected, string source)
    {
        var argument = TypedConstantFactory.Create(source);

        var result = Target(Context.Pattern, argument);

        Assert.Equal(expected, result.GetMatchedArgument());
    }

    [AssertionMethod]
    private static void Unsuccessful(string source)
    {
        var argument = TypedConstantFactory.Create(source);

        var result = Target(Context.Pattern, argument);

        Assert.False(result.Successful);
    }
}
